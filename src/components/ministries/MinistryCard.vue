<template>
  ...
  >
    ...
    <div class="relative h-48 w-full overflow-hidden bg-gray-200 dark:bg-gray-800">
      ...
      <!-- Icon Badge -->
      <div class="absolute left-4 top-4 flex h-12 w-12 items-center justify-center rounded-full bg-white/90 shadow-lg backdrop-blur-sm dark:bg-gray-900/90">
        <MaterialIcon :name="ministry.icon" class-name="text-2xl text-primary" />
      </div>

      <!-- Age Range Badge -->
      <div v-if="ministry.ageRange" class="absolute bottom-4 right-4 rounded-full bg-white/90 px-3 py-1 text-xs font-medium text-gray-700 backdrop-blur-sm dark:bg-gray-900/90 dark:text-gray-300">
        {{ ministry.ageRange }}
      </div>
    </div>
    ...
    <div class="p-6">
      <!-- Title -->
      <h3 class="mb-2 text-xl font-bold text-[#111418] dark:text-white">
        {{ ministry.name }}
      </h3>
      ...
      <!-- Leader Info -->
      <div class="mb-4 flex items-center gap-3 border-t border-gray-100 pt-4 dark:border-gray-700">
        ...
        <div class="flex-1">
          <p class="text-xs text-gray-500 dark:text-gray-400">Líder</p>
